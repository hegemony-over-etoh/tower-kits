...
return function(rootScope: _T.Scope, utility: _T.Utility, cache: { [Instance]: () -> () })
	...
	rootScope:spawn(utility.Functions.yieldTimeout, 5, function()
		sequencerRequest:request(SequencerCommunicator.REGISTER_OBJECT, "Teleporter", {
			...
			activate = function(sequenceData)
				local teleportFn = cache[sequenceData.instance]
				if typeof(teleportFn) == "function" then
					if sequenceData.canAwait then
						teleportFn()
					else
						task.spawn(teleportFn)
					end
				end
			end,
		})
