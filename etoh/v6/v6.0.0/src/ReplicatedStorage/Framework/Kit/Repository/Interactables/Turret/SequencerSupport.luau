...
return function(rootScope: _T.Scope, utility: _T.Utility, cache: { [Instance]: () -> () })
	...
	rootScope:spawn(utility.Functions.yieldTimeout, 5, function()
		sequencerRequest:request(SequencerCommunicator.REGISTER_OBJECT, "Turret", {
			...
			activate = function(sequenceData)
				local fireBullet = cache[sequenceData.instance]
				if typeof(fireBullet) == "function" then
					if sequenceData.canAwait then
						fireBullet()
					else
						task.spawn(fireBullet)
					end
				end
			end,
		})
