...
return function(rootScope: _T.Scope, utility: _T.Utility, cache: { [BasePart]: () -> () })
	...
	rootScope:spawn(utility.Functions.yieldTimeout, 5, function()
		sequencerRequest:request(SequencerCommunicator.REGISTER_OBJECT, "MusicZoneEditor", {
			check = function(sequenceData, optimize)
				local instance = sequenceData.instance
				local isEditor = cache[instance] ~= nil
				if isEditor then
					optimize(instance)
				end
			...
			activate = function(sequenceData)
				local editorFn = cache[sequenceData.instance]
				if typeof(editorFn) == "function" then
