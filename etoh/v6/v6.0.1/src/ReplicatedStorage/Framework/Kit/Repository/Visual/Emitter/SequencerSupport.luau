...
return function(rootScope: _T.Scope, utility: _T.Utility, cache: { [BasePart]: () -> () })
	...
	rootScope:spawn(utility.Functions.yieldTimeout, 5, function()
		sequencerRequest:request(SequencerCommunicator.REGISTER_OBJECT, "Emitter", {
			...
			activate = function(sequenceData)
				local emitterFn = cache[sequenceData.instance]
				if typeof(emitterFn) == "function" then
					if sequenceData.canAwait then
						emitterFn()
					else
						task.spawn(emitterFn)
					end
				end
			end,
		})
