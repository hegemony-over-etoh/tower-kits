...
return function(rootScope: _T.Scope, utility: _T.Utility, cache: { [BasePart]: (touch: BasePart) -> () })
	...
	rootScope:spawn(utility.Functions.yieldTimeout, 5, function()
		sequencerRequest:request(SequencerCommunicator.REGISTER_OBJECT, "Dismounter", {
			...
			activate = function(sequenceData)
				local characterParts = utility.Character.getCharacter()
				...
				if typeof(dismountFn) == "function" then
					if sequenceData.canAwait then
						...
						task.spawn(dismountFn, rootPart)
					end
				end
			end,
		})
