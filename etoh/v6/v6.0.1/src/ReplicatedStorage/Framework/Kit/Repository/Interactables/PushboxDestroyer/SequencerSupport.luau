...
return function(rootScope: _T.Scope, utility: _T.Utility, cache: { [BasePart]: () -> () })
	...
	rootScope:spawn(utility.Functions.yieldTimeout, 5, function()
		sequencerRequest:request(SequencerCommunicator.REGISTER_OBJECT, "PushboxDestroyer", {
			...
			activate = function(sequenceData)
				local destroyFn = cache[sequenceData.instance]
				if typeof(destroyFn) == "function" then
					if sequenceData.canAwait then
						destroyFn()
					else
						task.spawn(destroyFn)
					end
				end
			end,
		})
