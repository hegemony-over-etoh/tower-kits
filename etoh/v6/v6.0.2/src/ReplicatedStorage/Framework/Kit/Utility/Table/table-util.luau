...
local function Filter<I, T>(t: { [I]: T }, predicate: (T, I, { [I]: T }) -> boolean): { [I]: T }
	assert(type(t) == "table", "First argument must be a table")
	assert(type(predicate) == "function", "Second argument must be a function")
	local newT: any = table.create(#t)
	if #t > 0 then
		local n = 0
		for i, v in t do
...
local function Keys<K>(tbl: { [K]: unknown }): { K }
...
local function Values<V>(tbl: { [unknown]: V }): { V }
