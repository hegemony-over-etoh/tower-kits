...
return function(rootScope: _T.Scope, utility: _T.Utility, cache: { [BasePart]: () -> () })
	...
	rootScope:spawn(utility.Functions.yieldTimeout, 5, function()
		sequencerRequest:request(SequencerCommunicator.REGISTER_OBJECT, "ButtonDeactivator", {
			check = function(sequenceData, optimize)
				local instance = sequenceData.instance
				if not instance:IsA("BasePart") then
					return false
				end
				local isDeactivator = cache[instance] ~= nil
				if isDeactivator then
					optimize(instance)
				end
			...
			activate = function(sequenceData)
				local deactivatorFn = cache[sequenceData.instance]
				if typeof(deactivatorFn) == "function" then
					if sequenceData.canAwait then
						deactivatorFn()
					else
						task.spawn(deactivatorFn)
					end
				end
			end,
		})
